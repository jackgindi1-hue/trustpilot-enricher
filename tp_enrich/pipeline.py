...
def merge_enrichment_back_to_rows(df: pd.DataFrame, enriched_businesses: list) -> pd.DataFrame:
    ...
    wanted = [
        ...
        # PHASE 4.6: Component scores
        "canonical_score_name",
        "canonical_score_state",
        "canonical_score_domain",
        "canonical_score_phone",
...
def enrich_business(business_info: Dict, cache: EnrichmentCache, run_id: str = None) -> Dict:
    ...
    enriched_row = {
        ...
        # PHASE 4.6: Component scores for threshold tuning
        "canonical_score_name": enriched_data.get("canonical_score_name", 0.0),
        "canonical_score_state": enriched_data.get("canonical_score_state", 0.0),
        "canonical_score_domain": enriched_data.get("canonical_score_domain", 0.0),
        "canonical_score_phone": enriched_data.get("canonical_score_phone", 0.0),
        # PHASE 4.6 DISCOVERED: anchor discovery fields
...
def run_pipeline(
    ...
    # PHASE 4.6.1: Fix dtype warning by converting to string dtype before assignment
    if "company_search_name" in df.columns:
        df["company_search_name"] = df["company_search_name"].astype("string")
