...
## ✅ PHASE 4.6.5 — CANONICAL SCORE RESTORE + ANCHOR TRIGGER FIX (DEPLOYED)
...
### Files Changed
- `tp_enrich/adaptive_enrich.py` - 3 lines modified (anchor trigger OR logic)
- `.same/PHASE465_DEPLOYED.md` - deployment documentation created
- `.same/todos.md` - updated with Phase 4.6.5 status
### Expected Impact
...
**What it does:**
- Saves partial CSVs every 250 rows
- Enables downloading results before job completes
- Prevents data loss on crashes
**How it works:**
1. Every 250 businesses processed:
   - Write current enriched rows to partial CSV
   - Set `job.partial_available = True`
   - Update job metadata
2. Frontend shows "Download Partial" button when available
3. GET /jobs/:id/partial returns latest partial CSV
**Benefits:**
- ✅ Get results faster (don't wait for full completion)
- ✅ Resume-safe (can pick up from checkpoint)
- ✅ Better UX for large batches
### 4. Safe Phone JSON Handling
**What it does:**
- Handles NaN values in phone JSON fields
- Prevents JSON parse errors
- Gracefully falls back on bad data
**How it works:**
```python
try:
    phones = json.loads(row.get("phone_json") or "[]")
except:
    phones = []
```
**Benefits:**
...
## 🎯 NEXT PRIORITIES
### High Priority
1. **Test Phase 4.5 in production**
   - Upload test CSV with state-known businesses
   - Verify canonical matching works (≥80%)
   - Check entity matching logs
   - Confirm partial download works
2. **Monitor entity match quality**
   - Check `canonical_match_score` distribution
...
3. **UI improvements**
   ...
   - More detailed job status
   - Download format options (CSV, XLSX)
### Low Priority
1. **Code cleanup**
   - Remove dead code
